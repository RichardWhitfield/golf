...
| D35 | Markers | **A club logo in a fixed 44px chip**, or two initials — never both | Recognition without a hundred mismatched crests, and the chip is the hit target. A course with no logo takes an early return; a committed file that fails to decode takes the `error` listener, which swaps the broken image for the same letters — one code path, two failures. **Revised from the rank** (#42): 98 of the logos are transparent PNGs, so the number showed through the club's mark, and a bare rank was indistinguishable from a cluster count. Layering *initials* underneath would have fixed only the second half — `links-lady-bay-resort` is a dark mark on a transparent ground and letters bleed through it exactly as digits did — so the fallback is appended on failure, not hidden behind the logo. `courseInitials` is pure and lives in `domain/destinations.ts`. |
