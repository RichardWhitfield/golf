...
Add to `src/lib/domain/metrics.test.ts`:

```ts
import { BAND } from './scale'
import { bestOf } from './metrics'
...
```ts
import { CHARTED, METRICS, chartedInfo, isCharted, metricInfo } from './metrics'
...
import { METRICS, METRIC_FIELDS, chartedInfo } from './metrics'
